<?php
class Zedo_Auth_Adapter_Doctrine2 implements Zend_Auth_Adapter_Interface
{
    /**
                                ...
                                $credentialColumn = null)
    {
        ...
        if (null !== $entityName) {
            $entityNamespace = 'Application_Model_Entities_';
            $this->setEntityName($entityNamespace . ucwords($entityName));
        }
     ...
     *
     * @param  string $entityName
     * @return Zedo_Auth_Adapter_Doctrine2 Provides a fluent interface
     */
    ...
    protected function _authenticateSetup()
    {
        ...
        if ($this->_em === null) {
            ...
            $exception = 'A entity name must be supplied for the Zedo_Auth_Adapter_Doctrine2 authentication adapter.';
        } elseif ($this->_identityColumn == '') {
            $exception = 'An identity column must be supplied for the Zedo_Auth_Adapter_Doctrine2 authentication adapter.';
        } elseif ($this->_credentialColumn == '') {
            $exception = 'A credential column must be supplied for the Zedo_Auth_Adapter_Doctrine2 authentication adapter.';
        } elseif ($this->_identity == '') {
            $exception = 'A value for the identity was not provided prior to authentication with Zedo_Auth_Adapter_Doctrine2.';
        } elseif ($this->_credential === null) {
            $exception = 'A credential value was not provided prior to authentication with Zedo_Auth_Adapter_Doctrine2.';
        }
    ...
    {
        $dql  = 'SELECT u FROM ';
        $dql .= $this->_entityName;
        $dql .= ' u WHERE u.'; 
        $dql .= $this->_identityColumn;
        $dql .= ' = :email';
        
        $query = $this->_em->createQuery($dql)
                            ->setParameter('email', $this->_identity)
                            ;
    ...
    protected function _validateResult($resultIdentities)
    {
        if (count($resultIdentities) < 1) {
            ...
            $getCredentialColumn = 'get' . ucwords($this->_credentialColumn);
            if ($resultIdentity->{$getCredentialColumn}()  != $this->_credential) {
